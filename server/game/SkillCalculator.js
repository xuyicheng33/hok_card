...
class SkillCalculator {
  ...
  calculateDuoliyaSkill(casterId, targetId) {
    ...
    if (!caster || !target) {
      console.error('❌ [朵莉亚技能] 施法者或目标未找到', casterId, targetId);
    ...
    console.log(`✅ [朵莉亚-人鱼之赐] ${caster.card_name} 治疗 ${target.card_name}`);
    console.log(`   治疗量: ${actualHeal} (${oldHealth} → ${target.health}/${target.max_health})`);
  ...
  calculateLanSkill(casterId) {
    ...
    if (!caster) {
      console.error('❌ [澜技能] 施法者未找到', casterId);
    ...
    console.log(`✅ [澜-鲨之猎刃] ${caster.card_name} 攻击力提升`);
    console.log(`   攻击力: ${oldAttack} → ${caster.attack} (+${attackBuff})`);
    console.log(`   📊 服务器端卡牌状态: ${caster.card_name} ATK:${caster.attack}`);
  ...
  calculateGongsunliSkill(casterId) {
    ...
    if (!caster) {
      console.error('❌ [公孙离技能] 施法者未找到', casterId);
      return { success: false, error: '施法者未找到' };
    }
    ...
    console.log(`✅ [公孙离-晚云落] ${caster.card_name} 暴击属性提升`);
    console.log(`   暴击率: ${(oldCritRate*100).toFixed(1)}% → ${(newCritRate*100).toFixed(1)}%`);
    console.log(`   暴击效果: ${(oldCritDamage*100).toFixed(1)}% → ${(caster.crit_damage*100).toFixed(1)}%`);
    if (overflow > 0) {
      console.log(`   溢出转换: ${(overflow*100).toFixed(1)}% → ${(critDamageBonus*100).toFixed(1)}% 暴击效果`);
    }
    console.log(`   📊 服务器端卡牌状态: ${caster.card_name} 暴击${(caster.crit_rate*100).toFixed(1)}% 效果${(caster.crit_damage*100).toFixed(1)}%`);
  ...
  calculateSunshangxiangSkill(casterId, targetId) {
    ...
    return {
      ...
      target_health: target.health,
      target_shield: target.shield || 0,  // 🛡️ 同步护盾（虽然真伤不消耗）
    ...
    for (const enemy of enemyCards) {
      if (enemy && enemy.health > 0) {
        ...
        damageResults.push({
          ...
          target_shield: enemy.shield || 0,  // 🛡️ 同步护盾
  ...
  calculateShaosiyuanSkill(casterId, targetId, isAlly) {
    ...
    if (isAlly) {
      ...
      return {
        ...
        is_critical: isCrit,
        target_health: target.health,
        target_shield: target.shield || 0,  // 🛡️ 同步护盾
        target_dead: target.health <= 0
      };
    }
  }
