...
##
## 📦 模块化架构：
## - BattleUIManager: UI布局和自适应管理
## - BattleCombatHandler: 战斗执行和回合管理
##
## ⚠️ 当前状态：模块已创建，主脚本保持原有逻辑以确保稳定性
## 后续可逐步将功能迁移到对应模块

# 📦 子模块（已创建，预留for future integration）
var ui_manager: BattleUIManager  # UI管理器（预留）
var combat_handler: BattleCombatHandler  # 战斗处理器（预留）
